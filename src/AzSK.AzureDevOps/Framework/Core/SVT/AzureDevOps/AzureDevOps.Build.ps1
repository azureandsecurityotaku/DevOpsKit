...
{    
    ...
    [bool] CheckCredentials([string] $regexCode, [string] $valueToMatch)
    {
        $patterns = $this.ControlSettings.Patterns | where {$_.RegexCode -eq $regexCode} | Select-Object -Property RegexList;

        try {
            $apiURL = "https://extmgmt.dev.azure.com/{0}/_apis/ExtensionManagement/InstalledExtensions/ADOScanner/ADOSecurityScanner/Data/Scopes/Default/Current/Collections/MyCollection/Documents/OrgPolicy?api-version=5.1-preview.1" -f $($this.SubscriptionContext.SubscriptionName);
            $resObj = [WebRequestHelper]::InvokeGetWebRequest($apiURL);  
            $policyObj = $resObj.policy | ConvertFrom-Json;
            if( ($policyObj) -and ([Helpers]::CheckMember($policyObj,"Patterns")) ){

                $custPatternsRegList = $policyObj.Patterns | where {$_.RegexCode -eq $regexCode} | Select-Object -Property RegexList
                #for ($i = 0; $i -lt $custPatternsRegList.RegexList.Count; $i++) {
#
                #    if($patterns.RegexList -notcontains $custPatternsRegList.RegexList[$i])
                #    {
                #        $patterns.RegexList += $custPatternsRegList.RegexList[$i];
                #    }
                #}   
                #$patterns.RegexList += $custPatternsRegList.RegexList;        
                #$patterns.RegexList = $patterns.RegexList | select -Unique;  
                $patterns.RegexList = [Helpers]::MergeObjects( $patterns.RegexList, $custPatternsRegList)	
            }
        }
        catch {
            Write-Host $_
